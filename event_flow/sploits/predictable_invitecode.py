...
def get_event(host, token):
    try:
        ...
        return data['owner_username'], data['title'], data['id']
    except Exception as err:
        print(f"Error get eventing: {err}")
        sys.exit(1)


def join(token, id, invitecode):
    try:
        response = requests.post(
            f'http://{host}:{PORT}/api/event/{id}/join',
            json={
...
def run(): 
    ...
    token = register(host)
    owner_username, title, id = get_event(host, token)

    invitecode = make_invitecode(owner_username, title)
    join(token, id, invitecode)
